...
# %%
X_estacionario = celulas[:,0:,4].flatten()
Y_estacionario = celulas[:,sampling:,5].flatten()
Z_estacionario = celulas[:,sampling:,6].flatten()
# %%
informacion_instantanea= []
for posicion_temporal in np.arange(10,300):
    distribucion_X = celulas[:,0:,4][:, posicion_temporal]
    distribucion_Y = celulas[:,0:,5][:, posicion_temporal]
    distribucion_Z = celulas[:,0:,6][:, posicion_temporal]

    data = {'X': distribucion_X,
        'Y': distribucion_Y,
        'Z': distribucion_Z}
    ...
    informacion_instantanea.append(Informacion_Z_X_Y)
#%%
import matplotlib.pyplot as plt
plt.plot(informacion_futura)
plt.plot(informacion_instantanea)
# %%
plt.plot(np.array(informacion_futura) - np.array(informacion_instantanea))
# %%

# %%
celulas_prom = np.mean(celulas[:,0:,6],axis=0)

# %%
plt.plot(celulas_prom)
